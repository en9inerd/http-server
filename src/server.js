...
export function createServer(opts) {
  ...
  const handler = formHandlerChain([
...
async function handlePOSTFileRequest(req, res) {
  ...
  //
  const remoteAddr = req.socket.address().address;
  if (remoteAddr != '127.0.0.1' && remoteAddr != '::1') {
    return endResponse(res, 403, 'Forbidden');
  }

  const origin = req.headers.origin && urlparse(req.headers.origin).hostname;
  if (origin) {
...
async function serveDir(req, res, filename, st) {
  ...
  const indexFileSt = await stat(indexFile);
